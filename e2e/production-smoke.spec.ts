...
// Guard: all production smoke tests require a real https:// BASE_URL.
// When VERCEL_PRODUCTION_URL is not configured the env var arrives as an empty
// string, which would cause "Invalid URL" failures that look like production
// outages but are really a CI configuration gap.
const BASE_URL_ENV = process.env.BASE_URL ?? '';
const isProductionRun = BASE_URL_ENV.startsWith('https://');
...
test.describe('1. Infrastructure', () => {
  test.skip(
    !isProductionRun,
    'BASE_URL must be an https:// URL — set the VERCEL_PRODUCTION_URL repository variable to run production smoke tests'
  );
