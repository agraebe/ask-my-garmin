    ...
    <div className="flex flex-col items-center gap-6 px-3 py-8 sm:px-4 sm:py-12">
      <div className="text-center">
        ...
        <h2 className="text-xl font-semibold text-gray-900">
          {funMode ? 'RunningCircleJerk Mode' : 'Ask My Garmin'}
        </h2>
      ...
      <div className="grid w-full max-w-lg grid-cols-1 gap-2 sm:grid-cols-2">
        {suggestions.map((q) => (
          <button
            key={q}
            onClick={() => onSelect(q)}
            className={`rounded-xl border border-gray-200 bg-white px-3 py-3 text-left text-sm text-gray-700 shadow-sm transition-colors sm:px-4 ${funMode ? 'hover:border-rcj hover:bg-orange-50' : 'hover:border-garmin-blue hover:bg-blue-50'}`}
          >
            {q}
          </button>
        ))}
