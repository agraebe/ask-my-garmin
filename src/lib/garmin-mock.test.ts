...
describe('garmin-mock', () => {
  describe('getMockActivities', () => {
    ...
    it('should return different array instances each time', () => {
  ...
  describe('getMockRecoveryMetrics', () => {
    ...
    it('should have realistic recovery values', () => {
      ...
      recovery.forEach((metric) => {
        expect(metric.sleepScore).toBeGreaterThanOrEqual(0);
        expect(metric.sleepScore).toBeLessThanOrEqual(100);
        if (metric.avgOvernightHrv !== undefined) {
          expect(metric.avgOvernightHrv).toBeGreaterThan(20);
          expect(metric.avgOvernightHrv).toBeLessThan(120);
        }
      });
    });
    ...
    it('should have dates in chronological order', () => {
      ...
      for (let i = 1; i < recovery.length; i++) {
        const prevDate = new Date(recovery[i - 1].date);
        const currDate = new Date(recovery[i].date);
        expect(currDate.getTime()).toBeGreaterThan(prevDate.getTime());
      }
    });
  });
